...
UITextBox.prototype.keyCheck = function () {
    ...
    if (engine.keyboard.wasPressed("Backspace")) {
        if (txt.length > 0) {
            this.setText(txt.substring(0, txt.length - 1));
        }
    }
    engine.keyboard.wasPressedKeys.forEach(key => {
        console.log(key, "wasPressed");
        const not = [ "Backspace", "Enter"];
        if (not.includes(key)) return;
        this.setText(txt + "" + key);
        console.log(key, "writed");
    });
    if(engine.keyboard.wasPressed("Enter")){
...
UITextBox.prototype._applyUIXform = function(aCamera) {
    ...
    this.box.getXform().setPosition(WCPos[0] + (this.width+this.offset)/2, WCPos[1] + alignOff[1]);
    offset=this.mFontRenderable.getXform().getWidth()-this.width;
    if(offset<=0)
